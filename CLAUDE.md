...
- `plans/plan_YYYY_MM.json` — per-month schedule + day notes (`notes`) + per-assistant settings snapshot (`settings`: min/max shifts, max-consecutive, min-block, min-gap, oncall_attach)
