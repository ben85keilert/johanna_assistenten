...
**Usability is a hard requirement** (see BESCHREIBUNG.md): large click targets, side-by-side minus/plus buttons instead of tiny spin arrows, large font/rows. All sizes live centrally in **`ui/theme.py`** (`apply_theme(app)` sets font + stylesheet) — never hardcode sizes elsewhere.

- **MainWindow**: `QTabWidget` with tabs "Dienstplan" and "Team"; `on_close_save` callback triggers autosave in `main.py`. Hosts the **ControlBar** (`ui/control_bar.py`) between menu and tabs: an oversized value-control strip showing the last-activated `BigStepper` with huge ▲/▼ buttons and ◀/▶ field navigation (`_steppers_in_current_tab` enumerates targets).
...
- **TeamTab**: roster table with inline `BigStepper`s for target/max-consecutive/min-block (min/max kept mutually consistent), plus a chronological vacation list (add/edit/remove dialogs; ranges merge/split via the absence normalization). **ConstraintsDialog** remains for single unavailable days (calendar). Cross-tab sync: `TeamTab.assistants_changed` → `PlanTab.rebuild_grid`; `PlanTab.constraints_changed` → `TeamTab.refresh_all` (wired in `main.py`).
